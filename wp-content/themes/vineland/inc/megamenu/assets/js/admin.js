( function($) {
	var Vineland_Modal_Menu_Item = Backbone.View.extend({

		initialize: function( data ) {
			this.model = new Backbone.Model( data );
			this.template = '<span class="vineland-edit-menu"><i class="fa fa-pencil"></i> Mega</span>';
		...
		_openModalHandler: function() {
			window.vineland_menu_modal = new Vineland_Modal( this.model );
		}

	});

	// Backbone Modal View
	var Vineland_Modal = Backbone.View.extend({
		...
		initialize: function( model ) {
			...
			this.events = {
				...
				'click .vineland-modal-overlay' : '_closeHandler',
		...
		render: function() {
			var that = this;
			this.template = $( '#tpl-vineland-menu-item-modal' ).html();
			...
			data.action = 'vineland_load_menu_data';
			data.nonce = vineland_memgamnu_params.nonces.load_menu_data;
			...
			if ( this.model.get( 'is_loading' ) === true ) {
				$.ajax({
					type: 'POST',
					url: vineland_memgamnu_params.ajaxurl,
					data: data,
					beforeSend: function() {
		...
		_submitEditMenuForm: function( e ) {
			...
			data.push({
				name: 'action',
				value: 'vineland_update_menu_item_data'
			});
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
					form.find( '.vineland-modal-footer button' ).attr( 'disabled', true );
					form.find( 'button[type="submit"]' ).prepend( '<i class="fa fa-spin fa-spinner"></i> ' );
				}
			}).always( function() {
				form.find( '.vineland-modal-footer button' ).attr( 'disabled', false );
			} ).done( function( res ) {
				if ( res.status === true ) {
					that.model.destroy();
				}
	...
	$( document ).on( 'mouseenter', '#menu-to-edit .menu-item-handle', function(e) {
		...
		menu_title.append( new Vineland_Modal_Menu_Item({
			menu_id: menu_id,
			is_loading: true,
			istop:istop?1:0
		}).render().el );
		...
		var menu_item = $( this );
		var menu_title = menu_item.find( '.item-title' );
		menu_title.find( '.vineland-edit-menu' ).remove();
	} );
} )(jQuery);
